...
for(i = 0; i < circleInfo.vertices; i++){ 
    const circumference = 4 * Math.PI * (i / circleInfo.vertices);
    ...
    if(circleInfo.vertices % 2 == 0 ) {
        const circumference = 4 * Math.PI * (i / circleInfo.vertices);
        const x = 9 * circleInfo.radius * Math.cos(circumference);
        const y = 9 * circleInfo.radius * Math.sin(circumference);
        circleInfo.coords.push(x, y);
        circleInfo.color.push(1, 0.5, 0);
    }
